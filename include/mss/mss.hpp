...
private:
    ...
    void captureLoop() {
        const std::chrono::nanoseconds frame_duration_ns(16666667);// 约16.66667ms
        auto next_frame_time = std::chrono::steady_clock::now();

        while (_running.load()) {
            cv::Mat frame = grab(_captureMonitor);
            if (!frame.empty()) {
                {
            ...
            // 防止时间漂移，重置时间点
            if (std::chrono::steady_clock::now() > next_frame_time + frame_duration_ns) { next_frame_time = std::chrono::steady_clock::now(); }
        }
    }
...
public:
    ...
    // 启动持续截图线程，默认抓第0个显示器
    void start_capture(int monitor_index = 0) {
        if (_running.load()) {
    ...
    cv::Mat wait_and_get_frame(int timeout_ms = 100) {
        ...
        if (timeout_ms <= 0) { _frameCondVar.wait(lock, [this] { return !_latestFrame.empty(); }); } else { _frameCondVar.wait_for(lock, std::chrono::milliseconds(timeout_ms), [this] { return !_latestFrame.empty(); }); }
        return _latestFrame;
    }
    ...
    cv::Mat grab(const Monitor &monitor) {
        ...
        auto it = std::find_if(_outputDuplications.begin(), _outputDuplications.end(),
        ...
        cv::Mat fullImage = capture_output(*it);
        if (fullImage.empty()) {
...
protected:
    ...
    bool ensure_staging_texture(OutputDuplication &od) {
    ...
    cv::Mat capture_output(OutputDuplication &od) {
        ...
        while (retryCount < maxRetries) {
            ...
            cv::Mat image(od.monitor.height, od.monitor.width, CV_8UC4, mapped.pData, mapped.RowPitch);
